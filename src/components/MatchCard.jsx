...
function ScorerList({ scorers, align }) {
  if (!scorers || scorers.length === 0) {
    return <p className={`text-xs text-charcoal-600/70 italic ${align === 'right' ? 'text-right' : ''}`}>No goals</p>
  }
  return (
...
function TeamHero({ team, align }) {
  return (
    <div className={`flex flex-col items-center gap-2 flex-1 min-w-0 ${align === 'right' ? 'items-end' : 'items-start'} sm:items-center`}>
      ...
      <span className="font-display font-bold text-charcoal-900 text-sm truncate max-w-full">{team.name}</span>
      <FifaCode team={team} />
    </div>
  )
}
...
export default function MatchCard({ match, teamA, teamB, label, variant = 'default', onEdit, requireWinner = false }) {
  ...
  const editToggle = editable && !editing && (
    <SambaButton size="sm" variant="outline" onClick={() => setEditing(true)}>
      {match ? 'Edit Result' : 'Set Result'}
    </SambaButton>
  )

  if (variant === 'compact') {
    return (
      <div className="rounded-xl bg-white/80 border border-charcoal-900/10 shadow-depth px-3 py-2 space-y-2">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              {match ? (
                <div className="flex items-center justify-center shrink-0 px-1">
                  <div className="font-display font-extrabold text-base text-charcoal-900 tabular-nums whitespace-nowrap">
                ...
                <span className="text-xs text-charcoal-600 px-2">vs</span>
              )}
      ...
      <div
        className={`rounded-2xl p-5 shadow-depth-gold border-2 ${
          isFinal
            ...
            : 'bg-gradient-to-br from-charcoal-600/20 via-white to-charcoal-600/10 border-charcoal-600/40'
        }`}
      >
        <p className={`text-center font-display font-extrabold uppercase tracking-widest mb-3 ${isFinal ? 'text-charcoal-900 text-sm' : 'text-charcoal-600 text-xs'}`}>
          {isFinal ? '\u2605 Final \u2605' : '3rd Place Playoff'}
        </p>
        {editing ? editForm : (
          <>
            <div className="flex items-center justify-between gap-3">
              <TeamHero team={teamA} />
              <span className="font-display text-xs uppercase tracking-widest px-2 shrink-0">vs</span>
              <TeamHero team={teamB} align="right" />
            </div>
            {match && (
              <div className="flex items-center justify-center mt-4 pt-3 border-t border-charcoal-900/10">
  ...
  if (variant === 'hero') {
    return (
      <div className="rounded-2xl bg-white border-2 border-gold/50 shadow-depth-gold p-5">
        ...
        {editing ? editForm : (
          <>
            <div className="flex items-center justify-between gap-3">
              <TeamHero team={teamA} />
              <span className="font-display text-charcoal-600 text-xs uppercase tracking-widest px-2 shrink-0">vs</span>
              <TeamHero team={teamB} align="right" />
            </div>
            {editToggle && <div className="flex justify-center mt-3">{editToggle}</div>}
          </>
  ...
  if (!match && !editable) {
    return (
      <div className="rounded-2xl bg-white/80 border border-charcoal-900/10 shadow-depth p-4 flex items-center justify-between opacity-60">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <CountryFlag nation={teamA} size="sm" />
          <span className="text-sm font-medium text-charcoal-600 truncate">{teamA.name}</span>
        </div>
        <span className="font-display text-charcoal-600 text-sm px-2 shrink-0">vs</span>
  ...
  if (!match) {
    return (
      <div className="rounded-2xl bg-white/80 border border-charcoal-900/10 shadow-depth p-4 space-y-3">
        {editing ? editForm : (
          <>
  ...
  return (
    <div className="rounded-2xl bg-white border border-charcoal-900/10 shadow-depth p-4 space-y-3">
        ...
        <>
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <CountryFlag nation={teamA} size="sm" />
              <FifaCode team={teamA} />
              <span className="font-display font-medium text-charcoal-900 truncate">{teamA.name}</span>
            </div>

            <div className="flex items-center justify-center shrink-0 px-2">
              <div className="font-display font-extrabold text-2xl text-charcoal-900 tabular-nums whitespace-nowrap">
                {scoreA} &ndash; {scoreB}
                {wentToPenalties && (
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0 justify-end">
              <span className="font-display font-medium text-charcoal-900 truncate">{teamB.name}</span>
              <FifaCode team={teamB} />
              <CountryFlag nation={teamB} size="sm" />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-4 pt-3 border-t border-charcoal-900/10">
